...
public class KalahBoardPanel extends JPanel {
    ...
    public KalahBoardPanel(MancalaGame mancalaGameModel, MancalaUI mancalaGameUI) {
    ...
    private void setUpPanel() {
        ...
        headerPanel.add(new JLabel("K A L A H   G A M E", SwingConstants.CENTER));
    ...
    private void moveStones(int pitNum) {
        try {
            ...
            if (gameModel.isGameOver()) {
                ...
                if (gameModel.getBoard().isAnotherTurnIsRequired()) {
                    JOptionPane.showMessageDialog(this,
                            String.format("Player %s you have a bonus turn!", gameModel.getCurrentPlayer().getName()));
                } else {
    ...
    private void updatePlayerProfile(Player player, boolean isWinner, boolean isTie) {
        ...
        profile.addKalahGameCount();
        if (isWinner) {
            profile.addKalahWinCount();
        } else if (!isTie) {
            profile.addKalahLossCount();
        }
    ...
    private void saveGame() {
        try {
            Saver.saveObject(gameModel, gameUI.getKalahGameFile().toString());
            JOptionPane.showMessageDialog(this, "Kalah Game saved successfully.");
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Error saving Kalah Game: " + e.getMessage());
        }
    }

    private void loadGame() {
        try {
            gameModel = (MancalaGame) Saver.loadObject(gameUI.getKalahGameFile().toString());
            setPlayers(gameModel.getPlayer(1), gameModel.getPlayer(2));
            gameUI.getMainContainer().setPlayers(gameModel.getPlayer(1), gameModel.getPlayer(2));
            JOptionPane.showMessageDialog(this, "Kalah Game loaded successfully.");
        } catch (IOException e) {
            JOptionPane.showMessageDialog(this, "Error loading Kalah Game: " + e.getMessage());
        }
    }
}
